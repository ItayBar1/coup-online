...
export default class BlockDecision extends Component {
  ...
  render() {
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            {[
              ...
              const color = (INFLUENCE_DATA[claim] || {}).color || "#a88a86";
              return (
                <button
                  key={claim}
    ...
    if (action.action === "foreign_aid") {
      const color = INFLUENCE_DATA.duke.color;
      return (
        <div>
          <div
          ...
          <p className="font-body text-sm text-on-surface/80 mb-4 leading-relaxed">
            <span style={{ color: "#e87878" }} className="font-label">
              {action.source}
            </span>{" "}
    ...
    if (action.action === "assassinate") {
      const color = INFLUENCE_DATA.contessa.color;
      return (
        <div>
          <div
            className="font-label text-[10px] tracking-[0.4em] uppercase mb-3"
