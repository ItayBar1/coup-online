...
export default function SideNav({ name, activeTab, onTabChange, onTerminate }) {
  return (
    <aside className="fixed top-16 left-0 bottom-0 flex flex-col z-10 w-64 [@media(max-height:500px)]:w-14 bg-[#1a1208] border-r border-[#59413e]/15 shadow-[10px_0_30px_rgba(154,26,26,0.1)] transition-all duration-150">
      ...
      <div className="p-6 border-b border-outline-variant/10 [@media(max-height:500px)]:hidden">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="font-label text-xs tracking-widest text-primary truncate">
              {name}
            </p>
            <p className="font-label text-[10px] text-outline opacity-70">
              STATUS: ACTIVE
            </p>
          </div>
        </div>
      ...
      <nav className="flex-grow py-4 [@media(max-height:500px)]:py-1">
        {NAV_TABS.map((tab) => (
          ...
          >
            ...
            <span className="font-mono text-xs tracking-widest [@media(max-height:500px)]:hidden">
              {tab.label}
            </span>
          </button>
        ))}
      ...
      <div className="p-4 [@media(max-height:500px)]:p-1 border-t border-outline-variant/10">
        <button
          onClick={onTerminate}
          className="w-full flex items-center gap-4 [@media(max-height:500px)]:justify-center text-[#9a1a1a] p-4 [@media(max-height:500px)]:p-2 font-mono text-xs tracking-widest hover:bg-error-container/20 transition-all"
        >
